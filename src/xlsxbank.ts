...
function fillData(sheet: WorkSheet, headers: Header, movements: Array<Movements>, row: number): void {
    ...
    const dateValue =
        headers.date.column !== null ? sheet[utils.encode_cell({ c: headers.date.column, r: row })] : null;
    if (dateValue) {
        let cellDateV = dateValue.v;
        let cellDateW = dateValue.w;
        if (typeof cellDateV === 'string' && (cellDateV as string).includes('|')) {
            cellDateV = cellDateV.split('|')[0];
        }
        if (typeof cellDateW === 'string' && (cellDateW as string).includes('|')) {
            cellDateW = cellDateW.split('|')[0];
        }
        const validDateValue = cellDateV && validDate.test(cellDateV.toString().trim());
        const isValidDate = validDateValue || (cellDateW && validDate.test(cellDateW.toString().trim()));
        if (isValidDate) {
            const dateInput = validDateValue ? cellDateV.toString().trim() : cellDateW.toString().trim();
            let debitInput: string | number = '';
            let creditInput: string | number = '';
            const cellDescription =
                headers.desc.matched && headers.desc.column !== null
            ...
            if (headers.import.matched && headers.import.column !== null) {
                ...
                const cellDebit =
                    headers.debit.column !== null
                        ? sheet[utils.encode_cell({ c: headers.debit.column, r: row })]
                        : null;
                const cellCredit =
                    headers.credit.column !== null
